...
class Fabriek extends OefeningFactory
{
	public function run()
	{
		...
		if($tkt3 < 0 or $tkn3 < 0)
		{
			$opgave .= "\\left(\\frac{".$tkt3*$t3."}{".$tkn3*$n3."}\\right)";
		}else{
			$opgave .= "\\frac{".$tkt3*$t3."}{".$tkn3*$n3."}";
		}
		$oplossing = $opgave;
		//Vereenvoudig
		$teken = p($tkt1*$tkn1*$tkt2*$tkn2*$tkt3*$tkn3,0);
		...
		if(abs(ggd($t1,$n1))!=1 or abs(ggd($t2, $n2))!=1 or abs(ggd($t3,$n3))!=1)
		{
			...
			$t3 /= $ggd3;
			$n3 /= $ggd3;
			$oplossing .= "\\\\=$teken\\frac{".$t1."}{".$n1."}.\\frac{".$t2."}{".$n2."}.\\frac{".$t3."}{".$n3."}";
		}
		
		//Samen
		$oplossing .= "\\\\=$teken\\frac{".$t1.".".$t2.".".$t3."}{".$n1.".".$n2.".".$n3."}";
		if($teken == '+'){$teken = "";}
		//Uitwerken
		$teller = $t1 * $t2 *$t3;
		$noemer = $n1 * $n2 *$n3;
