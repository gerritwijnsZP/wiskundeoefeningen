...
class Fabriek extends OefeningFactory
{
	...
	private function oef11()
	{
		...
		if (($ga!=0)&($gb!=0)){
			...
			$opgave 	= $opg1;
			$oplossing	= $opgave;
			$oplossing .= "\\\\ r = \\sqrt{".getalafdruk($ga,1)."^2+".getalafdruk($gb,1)."^2} = ";
			$oplossing .= "\\sqrt{".pow($ga,2)+pow($gb,2)."} ";
			$hoek=atan($gb/$ga)*180/pi();
			if ($hoek<0){
				$hoek=$hoek+180;
			}
			$oplossing .= "\\\\ \\alpha = tan^{-1}(\\frac{".$gb."}{".$ga."}) \\Leftrightarrow \\alpha =".ddtodms($hoek)."\\text{ of } \\alpha = ".ddtodms($hoek+180);
